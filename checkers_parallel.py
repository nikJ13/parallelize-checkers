...
def helloworld(moves):
    #print(moves)
    value = Checkers.minimax(moves.get_board(),4, -math.inf, math.inf, False, False)
    return value
...
if __name__ == '__main__':
    checkers = Checkers()
    #checkers.play()
    print(f"{ansi_cyan}##### WELCOME TO CHECKERS ####{ansi_reset}")
    print("\nSome basic rules:")
    ...
    while True:
        ...
        if answer == "Y" or answer == "y":
            checkers.mandatory_jumping = True
            break
        elif answer == "N" or answer == "n":
            checkers.mandatory_jumping = False
            break
        elif answer == "":
            print(ansi_cyan + "Game ended!" + ansi_reset)
            exit()
    ...
    while True:
        ...
        if checkers.player_turn is True:
            print(ansi_cyan + "\nPlayer's turn." + ansi_reset)
            checkers.get_player_input()
        else:
           ...
           # for i in range(len(first_computer_moves)):
            ...
            #    value = Checkers.minimax(child.get_board(), 4, -math.inf, math.inf, False, checkers.mandatory_jumping)
                ######################################
            #    dict[value] = child
        
            #self.evaluate_states()
            ### FOR EVALUATING THE STATES
            t1 = time.time()
            current_state = Node(deepcopy(checkers.matrix))
            first_computer_moves = current_state.get_children(True, checkers.mandatory_jumping)
            if len(first_computer_moves) == 0:
                if checkers.player_pieces > checkers.computer_pieces:
                    print(
            ...
            p = pool.map(helloworld,first_computer_moves)
            for i in range(len(first_computer_moves)):
                child = first_computer_moves[i]
                dict[p[i]] = child
